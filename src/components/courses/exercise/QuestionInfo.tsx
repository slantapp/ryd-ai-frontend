...
export default function QuestionInfo({ question }: QuestionInfoProps) {
  ...
  return (
    <div className="min-w-0">
      ...
      {criteriaHint && (
        <p className="mt-2 min-w-0 max-w-full rounded border border-indigo-200 bg-indigo-50 p-2.5 text-xs text-indigo-900 wrap-anywhere sm:p-3 sm:text-sm">
          {criteriaHint}
        </p>
      )}
      {question.explanation && question.type === "code_test" && (
        <div className="mt-2 min-w-0 max-w-full rounded border border-blue-200 bg-blue-50 p-2.5 text-xs text-blue-800 wrap-anywhere sm:p-3 sm:text-sm">
